...
//   setPannerSpeed(2 * (settingState.setting['player.soundEffect.panner.speed'] / 10))
...
//       setConvolver(buffer, settingState.setting['player.soundEffect.convolution.mainGain'] / 10, settingState.setting['player.soundEffect.convolution.sendGain'] / 10)
//     })
//   }
...
//   }
// }
// type SEType = { [K in keyof AnyListen.AppSetting as K extends `player.soundEffect.${string}` ? K : never]: (val: AnyListen.AppSetting[K]) => void }
...
// export const initPlayerSettings = () => {
//   onRelease(unregistered.clear.bind(unregistered))
//   onPlayerCreated(() => {
//     unregistered.register(async(unregistered) => {
//       unregistered.add(onSettingChanged('player.volume', (val) => {
//         setVolume(val)
//       }))
//       unregistered.add(onSettingChanged('player.isMute', (val) => {
//         setMute(val)
//       }))
//       unregistered.add(onSettingChanged('player.playbackRate', (val) => {
//         setPlaybackRate(val)
//       }))
//       unregistered.add(onSettingChanged('player.preservesPitch', (val) => {
//         setPreservesPitch(val)
//       }))
//       unregistered.add(settingEvent.on('updated', (keys, settings) => {
